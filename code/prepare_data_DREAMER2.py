...
class PrepareData:
    ...
    def run(self, sub_clip_list , split=False, expand=True, feature=False):
        ...
        for sub in sub_clip_list[0]:
            data_, label_ = self.load_data_per_subject(sub,sub_clip_list[1])
    ...
    def load_data_per_subject(self, sub,clips):
        """
        ...
        new_size = 60000
        data = np.zeros((clips[-1]+1,14,new_size)) # 14 channels , len of signal is ?
        label = np.zeros((clips[-1]+1, 3))
        # 16768
        # 25472
        for i, clip in enumerate(clips):
            ...
            current_size = eeg_data.shape[1]

            # Calculate the amount of padding needed for each side of each row
            padding_needed = new_size - current_size
            padding_left = padding_needed // 2
            padding_right = padding_needed - padding_left

            # Pad each row of eeg_data with zeros
            eeg_data_padded = np.pad(eeg_data, ((0, 0), (padding_left, padding_right)), mode='constant')

            label[i] = np.array([valence, arousal, dominance])
            data[i] = eeg_data_padded

        # data = data.astype(np.float32)
        print(f"Pesron{sub+1}, Clips{clips}",end = '\t')
        print('data:' + str(data.shape) + ' label:' + str(label.shape) ,end = '\t')

        #   data: (clipsize, channels= 14, Signal_Length= 3000) 
        #   label: (clipsize, 3 ) #VAD
